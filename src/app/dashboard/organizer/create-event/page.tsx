...
export default function CreateEventPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (eventError || !events) throw eventError || new Error("Event insert returned no data.");
      ...
      const { error: ttError } = await supabase.from("ticket_types").insert(ttRows);
      if (ttError) {
        await supabase.from("events").delete().eq("id", eventId);
        throw ttError;
      }

      toast.success("Event and ticket types created successfully!");
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 text-sm text-zinc-400">
            ...
            <span className="text-zinc-700 dark:text-zinc-300 font-medium">Create Event</span>
          </div>
        </div>
